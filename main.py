...
while True:
    try:
        ...
        if(message.text != msg):
            ...
            #         sendButton.click()
            ...
            elif("!cc" in whatsapp_messege and whatsapp_messege != msg):
                msg= whatsapp_messege
                whatsapp_messege = whatsapp_messege.replace("!cc ","")
                url = f"https://competitive-programming-platform.p.rapidapi.com/codechef/{whatsapp_messege}"
                ...
                msg = finalReply
                finalReply=json.loads(finalReply)
                name = finalReply['user_details']['name']
                uname = finalReply['user_details']['username']
                rating = finalReply['rating']
                stars = finalReply['stars']
                grank = finalReply['global_rank']
                crank = finalReply['country_rank']
                # prblem = finalReply['fully_solved'][finalReply['fully_solved']['count']-1]
                msgBox = driver.find_element_by_xpath('//*[@id="main"]/footer/div[1]/div[2]/div/div[2]')
                msgBox.send_keys(f'name : {name} \n username : {uname} \n rating : {rating} \n stars : {stars} \n global rank : {grank} \n country rank : {crank}')
                sendButton = driver.find_element_by_xpath('//*[@id="main"]/footer/div[1]/div[3]/button')
                sendButton.click()
                ...
                for i in search(whatsapp_messege, num_results=11):
                    finalReply = str(i)
                    msg = finalReply
                    msgBox = driver.find_element_by_xpath('//*[@id="main"]/footer/div[1]/div[2]/div/div[2]')
                    msgBox.send_keys(finalReply)
            ...
            elif("!size of" in whatsapp_messege):
                    whatsapp_messege=whatsapp_messege.replace('!size of ','')
                    joke = ""
                    if(whatsapp_messege=='akash' or whatsapp_messege=='soham'):
                        joke = ':peach' u'\ue007'
                    else:
                ...
                url = f"https://api.pgamerx.com/v3/ai/response?message=encodeURIComponent('{whatsapp_messege}')&type=stable"

                querystring = {"message":"hellow there","language":""}
